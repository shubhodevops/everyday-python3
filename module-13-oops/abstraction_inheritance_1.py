...
# * সুবিধা: কোড সহজে পরিবর্তন করা যায় এবং ব্যবহারকারী শুধু কাজের অংশটি দেখতে পান। 
# যেমন: গাড়ি চালানোর সময় ব্রেক বা এক্সিলারেটর কীভাবে কাজ করে না জেনেও শুধু তা ব্যবহার করা।  



# 1. Abstraction (লুকিয়ে রাখা)Direct Answer: 
# Abstraction means hiding the complex background details and showing only the essential features to the user. 

# It answers the question: "What does this object do?" without worrying about 
# "How does it do it?" Real-Life Analogy: 
# The Coffee MachineWhen you want coffee, you just press a button. 

# You do not need to know how the machine heats the water, grinds the beans, or pumps the liquid. 

# The internal mechanics are abstracted (hidden) from you. 

# You only interact with the simple interface (the button).

# How it works in Python? Python uses the abc (Abstract Base Classes) module to create abstract classes.
...
# Using the code creating object:
...
# * সম্পর্ক তৈরি: ক্লাসগুলোর মধ্যে একটি Parent-Child বা Is-a সম্পর্ক তৈরি করে। 
# যেমন: Car ক্লাসটি Vehicle ক্লাস থেকে ইনহেরিট করতে পারে। 
...
# Think of a parent passing down traits like eye color, height, or last name to their child. The child automatically gets these traits but can also learn new skills or have unique hobbies that the parents do not have.
...
class ElectricCar(Vehicle):
    def __init__(self, brand, year, battery_capacity):
        ...
        self.battery_capacity = battery_capacity # Unique feature of child class


    def charge_battery(self): # Unique action of child class
        print(f"Charging the {self.brand}'s {self.battery_capacity}kWh battery.")

# Using the code create object
...
# * Inheritance: ফোকাস করে কোডের পুনঃব্যবহার এবং সম্পর্ক তৈরির ওপর। 
    ...
    # Keywords:   ABC, @abstractmethod (for abstract class)
    #             super(). (for inheritance class)
...
# Here is how you can implement this in Python by passing multiple parent classes 
...
# super() function inside Multiple Inheritance
#  
...
# In Single Inheritance, super() simply calls the parent class. But in Multiple Inheritance, super() does not just look at the direct parent. 

# Instead, it follows the Method Resolution Order (MRO) line.

# When you call super() inside a method, Python looks at the MRO chain and calls the next class in line, even if that class is not the direct parent of the current class!

#  This allows all parent classes to be initialized or called in a clean, cooperative order.
...
# No problem at all! Let us break it down step-by-step. 
# It is tricky because super() does something very special here.
# The correct answer is that three lines will print out in this exact order:
# "I fight with honor!"
...
# Python builds the lookup chain like this:Spellblade → Warrior → Mage → Character
# 
# Here is exactly what happens when you run player.speak():
# 
# Step 1: Python looks for speak() inside Spellblade. It is not there.

# Step 2: It goes to the next class in the chain, which is Warrior. 
# It runs Warrior.speak().🖨️ This prints: I fight with honor!

# Step 3: Inside Warrior.speak(), there is a super().speak() call. 
# Remember, super() means "go to the next class in the MRO chain". 

# The next class after Warrior is Mage!
